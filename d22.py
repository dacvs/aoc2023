import numpy as np
from topsort import topsort_checked
...
def d22(filename):
    with open(filename) as f:
        A = np.array([str2list(s) for s in f])
        A = A.reshape(-1, 2, 3)
        N = {}
        for u in range(len(A)):
            if u % 50 == 0:
                print(f"Computing neighborhoods {u} / {len(A)}")
            N[u] = list(nbhd(A, u))

        V = topsort_checked(N)  # first vertices of V are lowest
        for u in V:
            drop(A, N, u)
        return A, N, V
